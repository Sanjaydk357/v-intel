...
challan_cookie = "JSESSIONID=c2~824AF25B2AEDCC814928707E66A8B126"
...
head = {
    ...
    'Cookie': 'JSESSIONID=c2~49AEFD6DF6EBD34F9B97DADF924FBD73',
}
...
def load_cookies(driver, cookie_file):
    ...
    for cookie in cookies:
        # Fix missing/invalid sameSite value
...
def main(vehicle_number, output_file):
    try:
        ...
        # Load Chrome with driver
        driver = webdriver.Chrome(options=chrome_options)
        driver.get("https://www.carinfo.app")  # Open site to set cookies

        time.sleep(3)
        load_cookies(driver, cookie_file)

        # Now navigate to vehicle page
...
def fetch_challan(vehicle_number):
    ...
    try:
        ...
        with open(f"{vehicle_number}.txt", "+a") as file:
            if response.status_code == 200:
                ...
                if challan:
                    ...
                    if notice_numbers:
                        ...
                        print("\n✅ Media URLs saved to a file")
            else:
                if response.status_code == 403:
                    file.write(f"\nNo challans found for {vehicle_number} or session expired.\n")
                else:
                    if response.status_code == 429:
                        file.write("Error: Rate limit exceeded while fetching challans.")
                    else:
                        file.write(f"Error: Failed to retrieve challans, status code: {response.status_code}")
    except Exception as e:
        print(f"Error: {e}")
...
def extract_proof_urls(vehicle_number, notice_no):
    ...
    params = {
        "vehicleRegNumber": vehicle_number,
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 429:
                print("❌ Rate limit exceeded while fetching proof URLs.")
            else: print(f"❌ Failed to retrieve proof URLs, status code: {response.status_code}")
            return None
    except Exception as e:
        print(f"❌ Error retrieving proof: {e}")
        return None
...
def index():
    ...
    if request.method == "POST":
        ...
        if vehicle_number:
            ...
            # 1. Run your scraper functions
            main(vehicle_number, output_file) 
            fetch_challan(vehicle_number) # Fetches fines
    ...
    # Data containers
    media_links = {}
    challan_entries = [] # This will now store Dictionaries, not Strings
    ...
    if file_content:
        ...
        current_section = "VEHICLE" # VEHICLE, CHALLAN, MEDIA
        ...
        for line in lines:
            ...
            if current_section == "VEHICLE":
                # Store as is, or split if needed
                ...
                if line.startswith("Total Fine Amount:"):
                    ...
                    # CRITICAL FIX: Convert string representation to real Dictionary
                    try:
